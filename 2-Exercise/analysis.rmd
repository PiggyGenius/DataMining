...
We can confirm that by comparing our PCA with feature scaling and PCA without feature scaling, in the case of PCA without scaling our first two components capture 94.3% of the variance. For PCA with scaling, our first two components only capture around 0.3% of the variance.
...
We see that most populations are easily identified using the first and second components. A few populations are not really seperable since they fall on the same region (Wayuu, Huilliche, Arhuaco).
With the 5th and 6th axes, only two populations are easily identified: Karitiana and Ache.
```{r, echo=FALSE}
